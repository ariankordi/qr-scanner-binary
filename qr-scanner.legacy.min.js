...
c._preferredCamera&&(c._preferredCamera=a,yield c._restartVideoStream())})}static scanImage(a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d,f,g,k=!1,n=!1;c="object"===typeof c?c:{};d=c.scanRegion||d;f=c.qrEngine||f;g=c.canvas||g;k=c.disallowCanvasResizing||!1;n=c.alsoTryWithoutScanRegion||!1;let p=!!f;try{let q,m;[f,q]=yield Promise.all([f||b.createQrEngine(),b._loadImage(a)]);[g,m]=b._drawToCanvas(q,d,g,k);let r;if(f instanceof Worker){let h=f;p||b._postWorkerMessageSync(h,
"inversionMode","both");r=yield new Promise((l,w)=>{let x,v,t,z=-1;v=u=>{u.data.id===z&&(h.removeEventListener("message",v),h.removeEventListener("error",t),clearTimeout(x),null!==u.data.data?l({data:u.data.data,cornerPoints:b._convertPoints(u.data.cornerPoints,d)}):w(b.NO_QR_CODE_FOUND))};t=u=>{h.removeEventListener("message",v);h.removeEventListener("error",t);clearTimeout(x);w("Scanner error: "+(u?u.message||u:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",t);x=setTimeout(()=>
t("timeout"),1E4);let y=m.getImageData(0,0,g.width,g.height);z=b._postWorkerMessageSync(h,"decode",y,[y.data.buffer])})}else r=yield Promise.race([new Promise((h,l)=>window.setTimeout(()=>l("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=yield f.detect(g);if(!h)throw b.NO_QR_CODE_FOUND;return{data:h.rawValue,cornerPoints:b._convertPoints(h.cornerPoints,d)}}catch(l){h=l.message||l;if(/not implemented|service unavailable/.test(h))return b._disableBarcodeDetector=
!0,b.scanImage(a,{scanRegion:d,canvas:g,disallowCanvasResizing:k,alsoTryWithoutScanRegion:n});throw`Scanner error: ${h}`;}}))()]);return r}catch(q){if(!d||!n)throw q;return yield b.scanImage(a,{qrEngine:f,canvas:g,disallowCanvasResizing:k})}finally{p||b._postWorkerMessage(f,"close")}})}setGrayscaleWeights(a,c,d,f=!0){b._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:c,blue:d,useIntegerApproximation:f})}setInversionMode(a){b._postWorkerMessage(this._qrEnginePromise,"inversionMode",
...
Math.round(2/3*Math.min(a.videoWidth,a.videoHeight));return{x:Math.round((a.videoWidth-c)/2),y:Math.round((a.videoHeight-c)/2),width:c,height:c,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,k=a.offsetLeft,n=a.offsetTop,p=window.getComputedStyle(a),q=p.objectFit,m=c/d,r=f/g;switch(q){case "none":var h=c;var l=d;break;case "fill":h=
f;l=g;break;default:("cover"===q?m>r:m<r)?(l=g,h=l*m):(h=f,l=h/m),"scale-down"===q&&(h=Math.min(h,c),l=Math.min(l,d))}var [w,x]=p.objectPosition.split(" ").map((t,z)=>{const y=parseFloat(t);return t.endsWith("%")?(z?g-l:f-h)*y/100:y});p=this._scanRegion.width||c;r=this._scanRegion.height||d;q=this._scanRegion.x||0;var v=this._scanRegion.y||0;m=this.$overlay.style;m.width=`${p/c*h}px`;m.height=`${r/d*l}px`;m.top=`${n+x+v/d*l}px`;d=/scaleX\(-1\)/.test(a.style.transform);m.left=`${k+(d?f-w-h:w)+(d?c-
